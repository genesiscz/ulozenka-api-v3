...
use UlozenkaLib\APIv3\Model\TransportService\TransportService;
...
class JsonFormatterTest extends TestCase
{
	...
	public function testFormatGetTransportServicesResponse()
	{
		$json = file_get_contents(__DIR__ . '/data/getTransportServicesResponse.json');
		$connectorResponse = new ConnectorResponse(200, $json, []);
		$getTransportServiceBranchesResponse = $this->jsonFormatter->formatGetTransportServicesResponse($connectorResponse);
		$transportServices = $getTransportServiceBranchesResponse->getData();
		/** @var TransportService $firstTransportService */
		$firstTransportService = $transportServices[0];
		...
		Assert::type('\UlozenkaLib\APIv3\Model\Link', $link);
		Assert::same('self', $link->getResourceName());
		Assert::same('https://api.ulozenka.cz/v3/transportservices?shopId=5158', $link->getUrl());

		// errors
		Assert::same([], $errors);

		//transport services
		Assert::count(2, $transportServices);
		//transport service data
		Assert::type(TransportService::class, $firstTransportService);

		$transportServiceLinks = $firstTransportService->getLinks();
		Assert::count(2, $transportServiceLinks);
		$selfLink = $transportServiceLinks[0];
		Assert::same('self', $selfLink->getResourceName());
		Assert::same('https://api.ulozenka.cz/v3/transportservices/1', $selfLink->getUrl());
		$logoLink = $transportServiceLinks[1];
		Assert::same('logo', $logoLink->getResourceName());
		Assert::same('http://www.ulozenka.cz/src/images/ulozenka-logo.png', $logoLink->getUrl());

		Assert::same(1, $firstTransportService->getId());
		Assert::same('Uloženka', $firstTransportService->getName());
		Assert::same('', $firstTransportService->getDescription());
		Assert::same('ulozenka', $firstTransportService->getAlias());
		Assert::same(1, $firstTransportService->getActive());

		$transport = $firstTransportService->getTransport();
		Assert::same(1, $transport->getId());
		Assert::same('Uloženka', $transport->getName());
		Assert::same('ulozenka', $transport->getAlias());

		Assert::same(1, $firstTransportService->getUseDestinationBranches());

		$requirements = $firstTransportService->getRequirements();
		Assert::same(2, $requirements->getCompanyName());

		Assert::same(1, $requirements->getCustomerName());
		Assert::same(1, $requirements->getCustomerSurname());
		Assert::same(1, $requirements->getCustomerPhone());
		Assert::same(1, $requirements->getCustomerEmail());

		Assert::same(0, $requirements->getAddressStreet());
		Assert::same(0, $requirements->getAddressTown());
		Assert::same(0, $requirements->getAddressZip());
		Assert::same(1, $requirements->getAddressState());

		Assert::same(2, $requirements->getCashOnDelivery());
		Assert::same(0, $requirements->getStatedPrice());
		Assert::same(0, $requirements->getInsurance());
		Assert::same(2, $requirements->getCurrency());
		Assert::same(1, $requirements->getDestinationBranchId());
		Assert::same(2, $requirements->getVariable());
		Assert::same(2, $requirements->getPassword());
		Assert::same(2, $requirements->getParcelCount());
		Assert::same(2, $requirements->getNote());
		Assert::same(0, $requirements->getWeight());
		Assert::same(2, $requirements->getAllowCardPayment());
		Assert::same(2, $requirements->getRequireFullAge());
	}
