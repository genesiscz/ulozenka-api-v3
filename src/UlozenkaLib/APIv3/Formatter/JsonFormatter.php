...
use UlozenkaLib\APIv3\Model\TransportService\Requirements;
use UlozenkaLib\APIv3\Model\TransportService\Transport;
...
class JsonFormatter implements IFormatter
{
	...
	public function formatGetTransportServicesResponse(ConnectorResponse $connectorResponse)
	{
		...
		if ($responseCode === 200) {
			$data = $this->processTransportServicesResponseData($this->getJsonAttr($jsonObject, ResponseAttr::DATA));
		} else {
			$data = [];
		}
		$errors = $this->proccessResponseErrors($jsonObject);
		...
		$response = new GetTransportServicesResponse($rawResponseData, $responseCode, $links, $errors, $data);
	...
	protected function processTransportServicesResponseData($dataObject)
	{
		...
		foreach ($dataObject as $transportService) {
			$transportServiceObject = new \UlozenkaLib\APIv3\Model\TransportService\TransportService();
			$transportServiceLinks = $this->proccessLinks($this->getJsonAttr($transportService, '_links'));
			$transportServiceObject->setLinks($transportServiceLinks);
			$transportServiceObject->setId($this->getJsonAttr($transportService, 'id'));
			$transportServiceObject->setName($this->getJsonAttr($transportService, 'name'));
			$transportServiceObject->setDescription($this->getJsonAttr($transportService, 'description'));
			$transportServiceObject->setAlias($this->getJsonAttr($transportService, 'alias'));
			$transportServiceObject->setActive($this->getJsonAttr($transportService, 'active'));

			$transportData = $this->getJsonAttr($transportService, 'transport');
			$transportObject = new Transport();
			$transportObject->setId($this->getJsonAttr($transportData, 'id'));
			$transportObject->setName($this->getJsonAttr($transportData, 'name'));
			$transportObject->setAlias($this->getJsonAttr($transportData, 'alias'));

			$transportServiceObject->setTransport($transportObject);

			$transportServiceObject->setUseDestinationBranches($this->getJsonAttr($transportService, 'use_destination_branches'));

			$requirementsData = $this->getJsonAttr($transportService, 'requirements');
			$requirementsObject = new Requirements();
			$requirementsObject->setCompanyName($this->getJsonAttr($requirementsData, 'company_name'));
			$requirementsObject->setCustomerName($this->getJsonAttr($requirementsData, 'customer_name'));
			$requirementsObject->setCustomerSurname($this->getJsonAttr($requirementsData, 'customer_surname'));
			$requirementsObject->setCustomerPhone($this->getJsonAttr($requirementsData, 'customer_phone'));
			$requirementsObject->setCustomerEmail($this->getJsonAttr($requirementsData, 'customer_email'));
			$requirementsObject->setAddressStreet($this->getJsonAttr($requirementsData, 'address_street'));
			$requirementsObject->setAddressTown($this->getJsonAttr($requirementsData, 'address_town'));
			$requirementsObject->setAddressZip($this->getJsonAttr($requirementsData, 'address_zip'));
			$requirementsObject->setAddressState($this->getJsonAttr($requirementsData, 'address_state'));
			$requirementsObject->setCashOnDelivery($this->getJsonAttr($requirementsData, 'cash_on_delivery'));
			$requirementsObject->setStatedPrice($this->getJsonAttr($requirementsData, 'stated_price'));
			$requirementsObject->setInsurance($this->getJsonAttr($requirementsData, 'insurance'));
			$requirementsObject->setCurrency($this->getJsonAttr($requirementsData, 'currency'));
			$requirementsObject->setDestinationBranchId($this->getJsonAttr($requirementsData, 'destination_branch_id'));
			$requirementsObject->setVariable($this->getJsonAttr($requirementsData, 'variable'));
			$requirementsObject->setPassword($this->getJsonAttr($requirementsData, 'password'));
			$requirementsObject->setParcelCount($this->getJsonAttr($requirementsData, 'parcel_count'));
			$requirementsObject->setNote($this->getJsonAttr($requirementsData, 'note'));
			$requirementsObject->setWeight($this->getJsonAttr($requirementsData, 'weight'));
			$requirementsObject->setAllowCardPayment($this->getJsonAttr($requirementsData, 'allow_card_payment'));
			$requirementsObject->setRequireFullAge($this->getJsonAttr($requirementsData, 'require_full_age'));

			$transportServiceObject->setRequirements($requirementsObject);

			$transportServices[] = $transportServiceObject;
		}
